...
const SignupFormContent = () => {
  ...
  const onSubmit = async (values: SignupFormValues) => {
    ...
    try {
      ...
      // Extract password to send as passwordHash, delete password from payload
      ...
      router.push(selectedRole === 'INFLUENCER' ? '/influencer' : '/business');
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 flex items-center justify-center bg-zinc-50 dark:bg-zinc-950 p-6">
      ...
      >
        ...
        {error && (
          <div className="mb-6 p-4 rounded-xl bg-red-50 text-red-600 text-sm border border-red-100 italic font-medium">
            {error}
          </div>
        )}
            ...
            >
              ...
              {selectedRole === 'BUSINESS' && (
                <div>
                  ...
                  />
                  {errors.businessName && <p className="text-red-500 text-xs mt-1 px-1">{errors.businessName.message}</p>}
                </div>
              )}
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5 px-1">Mobile Number</label>
                <input
                  type="text"
                  {...register('phone')}
                  className="w-full px-5 py-3 rounded-2xl border border-zinc-200 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:bg-zinc-800 dark:border-white/5"
